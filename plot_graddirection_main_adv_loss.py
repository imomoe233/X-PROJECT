...
# 初始化列表存储每一步的梯度和损失
align_loss_clean_gradients = []
align_loss_backdoor_gradients = []
total_loss_history = []
total_loss_gradients = []
...
def plot_gradient_cosine_similarity(clean_grads, backdoor_grads, total_grads, save_path="gradient_cosine_similarity.png"):
    steps = len(total_grads)
    clean_similarities = []
    backdoor_similarities = []

    for i in range(steps):
        total_grad = total_grads[i]
        clean_grad = clean_grads[i]
        backdoor_grad = backdoor_grads[i]

        # 计算余弦相似度
        clean_similarity = 1 - cosine(total_grad, clean_grad)
        backdoor_similarity = 1 - cosine(total_grad, backdoor_grad)

        clean_similarities.append(clean_similarity)
        backdoor_similarities.append(backdoor_similarity)

    # 绘制余弦相似度曲线
    plt.figure(figsize=(10, 6))
    plt.plot(clean_similarities, label='Cosine Similarity with Align Loss Clean', color='blue')
    plt.plot(backdoor_similarities, label='Cosine Similarity with Align Loss Backdoor', color='red')

    plt.xlabel('Training Steps')
    plt.ylabel('Cosine Similarity')
    plt.title('Cosine Similarity Between Total Gradient and Individual Loss Gradients')
    plt.legend()
    plt.grid()
...
def plot_gradient_norms(clean_grads, backdoor_grads, total_grads, save_path="gradient_norms.png"):
    clean_norms = [np.linalg.norm(g) for g in clean_grads]
    backdoor_norms = [np.linalg.norm(g) for g in backdoor_grads]
    total_norms = [np.linalg.norm(g) for g in total_grads]

    plt.figure(figsize=(10, 6))
    plt.plot(clean_norms, label='Align Loss Clean Norm', color='blue')
    plt.plot(backdoor_norms, label='Align Loss Backdoor Norm', color='red')
    plt.plot(total_norms, label='Total Loss Norm', color='green')
...
def plot_gradient_paths_3d(clean_grads, backdoor_grads, total_grads, save_path="gradient_paths_3d.png"):
    # 将梯度列表转换为 NumPy 数组
    clean_grads_array = np.array(clean_grads)
    backdoor_grads_array = np.array(backdoor_grads)
    total_grads_array = np.array(total_grads)

    # 检查梯度形状是否一致
    assert clean_grads_array.shape[1] == backdoor_grads_array.shape[1] == total_grads_array.shape[1], "梯度向量长度不一致"

    # 使用 PCA 将梯度降维到三维
    pca = PCA(n_components=3)
    all_grads = np.vstack((clean_grads_array, backdoor_grads_array, total_grads_array))
    grads_3d = pca.fit_transform(all_grads)

    # 分割降维后的数据
    n = len(clean_grads)
    clean_grads_3d = grads_3d[:n]
    backdoor_grads_3d = grads_3d[n:2*n]
    total_grads_3d = grads_3d[2*n:]

    # 绘制梯度路径
    fig = plt.figure(figsize=(12, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(clean_grads_3d[:, 0], clean_grads_3d[:, 1], clean_grads_3d[:, 2], label='Align Loss Clean', color='blue', marker='o')
    ax.plot(backdoor_grads_3d[:, 0], backdoor_grads_3d[:, 1], backdoor_grads_3d[:, 2], label='Align Loss Backdoor', color='red', marker='x')
    ax.plot(total_grads_3d[:, 0], total_grads_3d[:, 1], total_grads_3d[:, 2], label='Total Loss', color='green', marker='s')

    ax.set_xlabel('PCA Component 1')
    ax.set_ylabel('PCA Component 2')
    ax.set_zlabel('PCA Component 3')
    ax.set_title('3D Gradient Paths via PCA')
    ax.legend()
    plt.grid()
...
def plot_gradient_paths_2d(clean_grads, backdoor_grads, total_grads, save_path="gradient_paths_2d.png"):
    # 将梯度列表转换为 NumPy 数组
    clean_grads_array = np.array(clean_grads)
    backdoor_grads_array = np.array(backdoor_grads)
    total_grads_array = np.array(total_grads)
    ...
    # 分割降维后的数据
    n = len(clean_grads)
    clean_grads_2d = grads_2d[:n]
    backdoor_grads_2d = grads_2d[n:2*n]
    total_grads_2d = grads_2d[2*n:]

    # 绘制梯度路径
    plt.figure(figsize=(10, 6))
    plt.plot(clean_grads_2d[:, 0], clean_grads_2d[:, 1], label='Align Loss Clean', color='blue', marker='o')
    plt.plot(backdoor_grads_2d[:, 0], backdoor_grads_2d[:, 1], label='Align Loss Backdoor', color='red', marker='x')
    plt.plot(total_grads_2d[:, 0], total_grads_2d[:, 1], label='Total Loss', color='green', marker='s')

    plt.xlabel('PCA Component 1')
    plt.ylabel('PCA Component 2')
    plt.title('2D Gradient Paths via PCA')
    plt.legend()
    plt.grid()
...
def plot_gradient_paths_incremental_pca(clean_grads, backdoor_grads, total_grads, n_components=2, batch_size=10, save_path="gradient_paths_ipca.png"):
    # 将梯度列表转换为 NumPy 数组
    clean_grads_array = np.array(clean_grads)
    backdoor_grads_array = np.array(backdoor_grads)
    total_grads_array = np.array(total_grads)
    ...
    ipca.fit(all_grads)
    grads_reduced = ipca.transform(all_grads)

    # 分割降维后的数据
    n = len(clean_grads)
    clean_grads_reduced = grads_reduced[:n]
    backdoor_grads_reduced = grads_reduced[n:2*n]
    total_grads_reduced = grads_reduced[2*n:]

    # 绘制梯度路径（以二维为例）
    plt.figure(figsize=(10, 6))
    plt.plot(clean_grads_reduced[:, 0], clean_grads_reduced[:, 1], label='Align Loss Clean', color='blue', marker='o')
    plt.plot(backdoor_grads_reduced[:, 0], backdoor_grads_reduced[:, 1], label='Align Loss Backdoor', color='red', marker='x')
    plt.plot(total_grads_reduced[:, 0], total_grads_reduced[:, 1], label='Total Loss', color='green', marker='s')

    plt.xlabel('Component 1')
    plt.ylabel('Component 2')
    plt.title('Gradient Paths via Incremental PCA')
    plt.legend()
    plt.grid()
...
def plot_combined_gradient_descent_paths(
    align_loss_clean_gradients,
    align_loss_backdoor_gradients,
    total_loss_gradients,
    save_path="plots/combined_gradient_descent_paths.png"
):
    # 将梯度列表转换为 NumPy 数组
    align_loss_clean_gradients_flat = np.array(align_loss_clean_gradients)
    align_loss_backdoor_gradients_flat = np.array(align_loss_backdoor_gradients)
    total_loss_gradients_flat = np.array(total_loss_gradients)

    # 使用 PCA 将所有梯度降维到 2D
    pca = PCA(n_components=2)
    all_gradients_flat = np.vstack((
        align_loss_clean_gradients_flat,
        align_loss_backdoor_gradients_flat,
        total_loss_gradients_flat
    ))
    grads_2d = pca.fit_transform(all_gradients_flat)
    
    # 分别提取每条路径的降维结果
    n = len(align_loss_clean_gradients_flat)
    clean_grads_2d = grads_2d[:n]
    backdoor_grads_2d = grads_2d[n:2*n]
    total_grads_2d = grads_2d[2*n:]
...
def plot_loss_curve(total_loss_history, save_path="plots/total_loss_descent_curve.png"):
    """
    绘制总损失的下降曲线，并保存图像。
    
    参数:
    - total_loss_history: 每一步的总损失值列表
    - save_path: 保存路径
    """
    ...
    plt.ylabel('Loss')
    plt.title('Total Loss Descent Curve')
    plt.legend()
    plt.grid()
...
def get_flattened_gradients(gradients, param_shapes):
    """
    将梯度展平为一个一维 NumPy 数组，替换 None 梯度为零向量。
    """
    flat_grads = []
    for g, shape in zip(gradients, param_shapes):
        if g is not None:
            flat_grads.append(g.detach().cpu().numpy().reshape(-1))
        else:
            # 如果梯度为 None，则用零向量替代
            flat_grads.append(np.zeros(shape.numel()))
...
def train_net_fedcon_backdoor(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    # 在训练开始前，获取所有参数的形状
    param_shapes = [param.data.shape for param in net.parameters()]
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                #loss = align_loss_clean + align_loss_backdoor + fedprox_reg + loss_cls
                #loss = loss_cls
                ...
                if batch_idx % 2 == 0:
                    optimizer.zero_grad()
                    loss.backward(retain_graph=True)  # 计算总损失的梯度
                    total_grads = get_flattened_gradients([param.grad for param in net.parameters()], param_shapes)
                    total_loss_gradients.append(total_grads)

                    # 获取 align_loss_clean 的梯度方向
                    optimizer.zero_grad()
                    align_loss_clean.backward(retain_graph=True)
                    clean_grads = get_flattened_gradients([param.grad for param in net.parameters()], param_shapes)
                    align_loss_clean_gradients.append(clean_grads)

                    # 获取 align_loss_backdoor 的梯度方向
                    optimizer.zero_grad()
                    align_loss_backdoor.backward(retain_graph=True)
                    backdoor_grads = get_flattened_gradients([param.grad for param in net.parameters()], param_shapes)
                    align_loss_backdoor_gradients.append(backdoor_grads)
                else:
                ...
                if batch_idx % 10 == 0:
                    # 进行优化步骤
                    optimizer.zero_grad()  # 再次清空梯度确保下一步更新的正确性
                    loss.backward()  # 重新计算所有梯度用于优化
...
def backdoor_MCFL(args):
    ...
    if args.backdoor == 'backdoor_MCFL':
        ...
        for round in range(n_comm_rounds):
            ...
            if round != 0:
                # 调用绘图方法
                #plot_gradient_paths_incremental_pca(align_loss_clean_gradients, align_loss_backdoor_gradients, total_loss_gradients)
                
                #plot_gradient_paths_2d(align_loss_clean_gradients, align_loss_backdoor_gradients, total_loss_gradients, save_path="plots/gradient_paths_2d.png")

                plot_gradient_norms(
                    align_loss_clean_gradients,
                    align_loss_backdoor_gradients,
                    total_loss_gradients,
                    save_path="plots/gradient_norms.png"
                )
                plot_gradient_cosine_similarity(
                    align_loss_clean_gradients,
                    align_loss_backdoor_gradients,
                    total_loss_gradients,
                    save_path="plots/gradient_coss.png"
                )
