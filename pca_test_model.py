...
parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\cifar10_resnet50\MCFL/fedavg_30.pth', help='the model to load as global model')
...
title = 'PCA-3D Visualization of cifar10 and Backdoor Samples'
...
for i in range(10):
    ...
    ax.scatter(all_features_3d[indices, 0], all_features_3d[indices, 1], all_features_3d[indices, 2], 
               label=f'Class {i}', color=colors[i], alpha=0.7)

# 绘制后门类
backdoor_indices = np.where(all_labels == 10)
ax.scatter(all_features_3d[backdoor_indices, 0], all_features_3d[backdoor_indices, 1], all_features_3d[backdoor_indices, 2], 
           label='Backdoor Class', color='black', alpha=0.2)
