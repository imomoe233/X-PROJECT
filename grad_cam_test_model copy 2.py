...
parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\MNIST_resnet50\FL\weight_fedavg.pth', help='the model to load as global model')
...
for idx in range(num_samples):
    ...
    # 将灰度图像转换为 BGR 彩色图像（如果是灰度图像）
    if len(input_image_uint8.shape) == 2 or input_image_uint8.shape[2] == 1:
        input_image_uint8 = cv2.cvtColor(input_image_uint8, cv2.COLOR_GRAY2BGR)

    # 叠加热力图和原始图像
    superimposed_img = cv2.addWeighted(heatmap_colored, 0.6, input_image_uint8, 0.4, 0)
