...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='resnet50', help='neural network used in training, resnet50,resnet50-MNIST')
    ...
    parser.add_argument('--defence', type=str, required=False, default="lockdown", help="lockdown")
    ...
    #parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\cifar10_resnet18/backdoor_pretrain(cleanOnly).pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='Y:\FRDA\model\cifar10_resnet18/cifar10_resnet18_backdoor_pretrain(triggerOnly)_199.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\cifar100_resnet18\cifar100_resnet18_backdoor_pretrain(cleanOnly)_60.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='Y:\FRDA\model\cifar100_resnet18/backdoor_pretrain(triggerOnly).pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\MNIST_resnet18/MNIST_resnet18_backdoor_pretrain(cleanOnly)_50.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='Y:\FRDA\model\MNIST_resnet18/backdoor_pretrain(triggerOnly).pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_simple-cnn\cifar10_simple-cnn_backdoor_pretrain(cleanOnly)_90.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_simple-cnn\cifar10_simple-cnn_backdoor_pretrain(triggerOnly)_35.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_simple-cnn\cifar100_simple-cnn_backdoor_pretrain(cleanOnly)_40.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_simple-cnn\cifar100_simple-cnn_backdoor_pretrain(triggerOnly)_69.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='Y:\FRDA\model\MNIST_simple-cnn\MNIST_simple-cnn_backdoor_pretrain(cleanOnly)_20.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='Y:\FRDA\model\MNIST_simple-cnn\MNIST_simple-cnn_backdoor_pretrain(triggerOnly)_25.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_vgg16\cifar10_vgg16_backdoor_pretrain(cleanOnly)_3490.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_vgg16\cifar10_vgg16_backdoor_pretrain(triggerOnly)_39.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_vgg19\cifar10_vgg19_backdoor_pretrain(cleanOnly)_1660.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_vgg19\cifar10_vgg19_backdoor_pretrain(triggerOnly)_50.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_vgg16\cifar100_vgg16_backdoor_pretrain(cleanOnly)_610.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_vgg16\cifar100_vgg16_backdoor_pretrain(triggerOnly)_190.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_vgg19\cifar100_vgg19_backdoor_pretrain(cleanOnly)_540.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_vgg19\cifar100_vgg19_backdoor_pretrain(triggerOnly)_190.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_vgg16\MNIST_vgg16_backdoor_pretrain(cleanOnly)_30.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_vgg16\MNIST_vgg16_backdoor_pretrain(triggerOnly)_100.pth', help='the model to load as global model')
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_vgg19\MNIST_vgg19_backdoor_pretrain(cleanOnly)_10.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_vgg19\MNIST_vgg19_backdoor_pretrain(triggerOnly)_50.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_densenet\cifar10_densenet_backdoor_pretrain(cleanOnly)_650.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar10_densenet\cifar10_densenet_backdoor_pretrain(triggerOnly)_20.pth', help='the model to load as global model')
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_densenet\cifar100_densenet_backdoor_pretrain(cleanOnly)_370.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\cifar100_densenet\cifar100_densenet_backdoor_pretrain(triggerOnly)_190.pth', help='the model to load as global model')
    
    
    #parser.add_argument('--load_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_densenet\MNIST_densenet_backdoor_pretrain(cleanOnly)_320.pth', help='the model to load as global model')
    #parser.add_argument('--load_backdoor_model_file', type=str, default='X:\Directory\code\MOON-backdoor\models\MNIST_densenet\MNIST_densenet_backdoor_pretrain(triggerOnly)_190.pth', help='the model to load as global model')
    ...
    parser.add_argument('--comm_round', type=int, default=800, help='number of maximum communication roun')
    ...
    #args.load_model_file = None
    #args.load_backdoor_model_file = None
    if args.wandb:
        wandb.init(
            # set the wandb project where this run will be logged
            project="MCFL-backdoor",
            #name=args.partition + '_' + 'clients' + str(args.n_parties) + args.log_file_name,
            name=args.defence + '_' + args.partition + '_' +  args.log_file_name,
            config={
            'epochs': args.epochs,
...
def apply_br_trigger(images, mask, pattern, square_size=5):
    """
    将可学习的 mask + pattern 叠加到图像的右下角 (square_size x square_size) 区域。

    参数:
        images:   shape (N, C, H, W) 的原图像
        mask:     shape (1, C, square_size, square_size)
        pattern:  shape (1, C, square_size, square_size)
        square_size: 5

    返回:
        triggered_images: shape (N, C, H, W)，含触发器
    """
    N, C, H, W = images.shape

    # clamp 到合法范围(避免梯度更新后mask<0或>1)
    mask_clamped = torch.clamp(mask, 0, 1)
    pattern_clamped = torch.clamp(pattern, 0, 255)  # 如果您图像是[0,1]可用 clamp(...,0,1)

    # 在右下角的坐标范围
    h_start = H - square_size
    w_start = W - square_size

    # 先复制一份
    triggered = images.clone()

    # broadcast: mask与pattern都是(1, C, s, s), 需要扩充到(N, C, s, s)
    mask_broadcasted = mask_clamped.expand(N, -1, -1, -1)      # (N, C, s, s)
    pattern_broadcasted = pattern_clamped.expand(N, -1, -1, -1)

    # 将 images[..., h_start:, w_start:] 替换成 (1 - mask) * 原像素 + mask * pattern
    original_patch = triggered[:, :, h_start:, w_start:]               # shape (N, C, s, s)
    new_patch = (1 - mask_broadcasted) * original_patch + mask_broadcasted * pattern_broadcasted
    triggered[:, :, h_start:, w_start:] = new_patch

    return triggered

def neural_cleanse_br5(
    model,
    dataloader,
    device,
    num_classes=10,
    ...
    lr=0.1,
    reg_coeff=1e-3,
    pixel_upper=255.0  # 若图像原本是0-1范围,可设1.0
):
    """
    在Neural Cleanse框架下，只允许(可学习的)mask+pattern在右下角 square_size x square_size区域激活。

    参数:
        model: 已训练好的模型 (nn.Module)
        dataloader: 用于检测的验证集数据 DataLoader (不含后门)
        device: 'cuda' or 'cpu'
        num_classes: 类别数
        square_size: 触发器所在的区域大小(5x5)
        max_steps: 每个类别要优化多少步
        lr: 学习率
        reg_coeff: L1正则系数
        pixel_upper: pattern最大取值(若图片是[0,1],设1.0;若是[0,255],设255)
    返回:
        all_results: dict 记录每个类别学到的{'mask':..., 'pattern':..., 'size':... }
        suspicious_label: 若某个类别对应mask特别小，则返回它，否则None
    """
    model.eval().to(device)

    # 准备将 DataLoader 的若干图像合并做“参考集”
    # 真实实现常只取部分批次，以减小计算量
    ref_images_list = []
    ref_labels_list = []
    for (x, y) in dataloader:
        ref_images_list.append(x)
        ref_labels_list.append(y)
        # 可限制只取前 100~200张, 这里示例取完
    ref_images = torch.cat(ref_images_list, dim=0).to(device)
    ref_labels = torch.cat(ref_labels_list, dim=0).to(device)

    # 统计一下
    N = ref_images.size(0)
    print(f"[Info] Using {N} reference images for detection")
    ...
    # 获取图像的通道数
    _, C, H, W = ref_images.shape

    # 对每个目标标签 t 做一次优化
    for target_label in range(num_classes):
        # 初始化可学习的 mask, pattern
        # shape: (1, C, square_size, square_size)
        # 这里初始化为随机,也可初始化成0
        mask = nn.Parameter(torch.zeros((1, C, square_size, square_size), device=device))
        pattern = nn.Parameter(torch.randn((1, C, square_size, square_size), device=device) * (pixel_upper/2) + (pixel_upper/2))
        ...
        for step in range(max_steps):
            ...
            # 构造带触发器的图像
            triggered_imgs = apply_br_trigger(ref_images, mask, pattern, square_size=square_size)

            # 前向
            logits = model(triggered_imgs)
            # 希望全部预测成 target_label
            target_tensor = torch.LongTensor([target_label]*N).to(device)
            loss_ce = ce_loss_fn(logits, target_tensor)

            # L1 正则 (只对mask做稀疏度约束)
            l1_reg = torch.norm(mask, p=1)

            # 总loss
            loss = loss_ce + reg_coeff * l1_reg
            loss.backward()
            optimizer.step()

            if (step+1) % 50 == 0:
                print(f"[Label={target_label}] step={step+1}, loss={loss.item():.4f}, CE={loss_ce.item():.4f}, L1={l1_reg.item():.4f}")

        # 迭代结束，记录结果
        final_mask = torch.clamp(mask.detach(), 0, 1)
        final_pattern = torch.clamp(pattern.detach(), 0, pixel_upper)
        # mask size = mask的 L1 范数(或 sum)
        mask_size = final_mask.abs().sum().item()

        all_results[target_label] = {
            "mask": final_mask.cpu(),
            "pattern": final_pattern.cpu(),
            "mask_size": mask_size
        }
    ...
    std_size = np.std(mask_sizes)

    suspicious_label = None
    for t in range(num_classes):
        if mask_sizes[t] < (mean_size - 2 * std_size):
            suspicious_label = t
            break

    if suspicious_label is not None:
        print(f"[Neural Cleanse (BR5)] Suspicious label={suspicious_label}, mask size={mask_sizes[suspicious_label]:.4f}")
    else:
        print("[Neural Cleanse (BR5)] No significantly small mask found.")

    return all_results, suspicious_label

def get_model_shapes_and_numels(model_state_dict):
    """
    返回:
      shapes: [ (层0shape), (层1shape), ... ]
      numels: [ 层0元素个数, 层1元素个数, ...]
    """
    shapes = []
    numels = []
...
def flatten_model_params(model_state_dict):
    """
    将 model_state_dict 中所有参数展平并拼成一个 1D 大向量
    返回: big_tensor(1D), shapes(list), numels(list)
    """
    shapes, numels = get_model_shapes_and_numels(model_state_dict)
    flattened_list = []
    for key, param in model_state_dict.items():
        flat = param.view(-1)
        flattened_list.append(flat)
    big_tensor = torch.cat(flattened_list, dim=0)
    return big_tensor, shapes, numels

def unflatten_model_params(flattened_tensor, shapes, numels):
    """
    将 1D 向量 flattened_tensor 拆分回各层形状
    返回 list_of_tensors, 对应每层
    """
    list_of_tensors = []
    offset = 0
    for shape, n_e in zip(shapes, numels):
        flat_slice = flattened_tensor[offset : offset + n_e]
        t = flat_slice.view(shape)
        list_of_tensors.append(t)
        offset += n_e
    return list_of_tensors
def extract_subspace_pca(updates, k=10):
    """
    给定 updates shape = [N, d]
    用 PCA 提取前 k 个主成分 (basis)。

    返回:
      mean_update: [d], updates 的均值
      basis:       [d, k], 主成分矩阵 (每列1个主成分)
    """
    # 1) 均值中心化
    mean_update = torch.mean(updates, dim=0, keepdim=True)  # shape=[1,d]
    U_centered = updates - mean_update

    # 2) SVD
    # Torch >=1.9: torch.linalg.svd, 也可用 torch.svd
    # U_centered shape=[N,d], 可能 N < d
    # 这里直接做全尺寸SVD示例
    U_, S_, Vh_ = torch.linalg.svd(U_centered, full_matrices=False)
    # Vh_.shape=[d,d], 取前k行 => basis为 [k,d], 需转置成 [d,k]
    basis = Vh_[:k, :].T.contiguous()  # [d, k]

    # 返回 (mean, basis)
    return mean_update.squeeze(0), basis
def project_to_subspace(delta, mean_update, basis):
    """
    delta: [d]
    mean_update: [d]
    basis: [d, k]
    ...
    """
    # center
    centered = delta - mean_update
    # 计算系数
    coeff = torch.matmul(basis.T, centered)  # [k]
    # 重建
    reconstructed = mean_update + torch.matmul(basis, coeff)  # [d]
    return reconstructed
def lockdown_aggregate(global_model, nets_this_round, net_dataidx_map, k=10, device='cuda:0'):
    """
    简化 Lockdown 实现示例:
      1) 计算每个客户端 delta = net - global
      2) flatten => all_updates_tensor
      3) PCA => basis
      4) 投影 => delta_proj
      5) 加权平均 => d_total
      6) unflatten => 更新 global_model

    参数:
      global_model: 当前全局模型 (nn.Module)
      nets_this_round: dict 或 list, 存每个客户端模型
      net_dataidx_map: dict, 存每个客户端的数据量(用于加权)
      k: 子空间维度
    """
    global_model.eval()
    ...
    # 记录 shapes, numels
    shapes, numels = get_model_shapes_and_numels(global_w)
    total_dim = sum(numels)

    # 收集所有客户端的flatten更新
    all_updates_list = []
    all_sizes = []
    client_ids = list(nets_this_round.keys()) if isinstance(nets_this_round, dict) else list(range(len(nets_this_round)))

    for cid in client_ids:
        net = nets_this_round[cid]

        net_para = net.state_dict()
        # delta_dict = net_para - global_w
        delta_dict = {}
        for key in net_para:
            delta_dict[key] = net_para[key] - global_w[key]

        delta_flat, _, _ = flatten_model_params(delta_dict)
        all_updates_list.append(delta_flat.to(device))
        # 记录该客户端的数据量(或权重)
        all_sizes.append(len(net_dataidx_map[cid]))

    # stack => shape=[N, d]
    all_updates_tensor = torch.stack(all_updates_list, dim=0).to(device)
    all_sizes = torch.tensor(all_sizes, dtype=torch.float32, device=device)
    total_data_points = all_sizes.sum()

    # =========== 提取子空间 (PCA) ============
    mean_update, basis = extract_subspace_pca(all_updates_tensor, k=k)

    # =========== 投影并做加权平均 =============
    d_total = torch.zeros(total_dim, device=device)
    for i in range(all_updates_tensor.size(0)):
        delta_i = all_updates_tensor[i]
        w_i = all_sizes[i] / total_data_points

        # 投影
        delta_proj_i = project_to_subspace(delta_i, mean_update, basis)
        # 加权累加
        d_total += w_i * delta_proj_i

    # 这样就得到最终聚合后的更新向量 d_total: shape=[d]

    # =========== 拆分并加回到 global_w ===========
    restored_tensors = unflatten_model_params(d_total, shapes, numels)

    new_global_dict = {}
    idx_layer = 0
    for (key, old_param) in global_w.items():
        shaped_delta = restored_tensors[idx_layer]
        new_global_dict[key] = (old_param + shaped_delta).to(device)
        idx_layer += 1

    # 更新全局模型
    global_model.load_state_dict(new_global_dict)
    print("[Lockdown] Aggregation done, updated global_model.")
...
def init_nets(net_configs, n_parties, args, device='cpu'):
    ...
    if args.dataset in {'MNIST', 'mnist', 'cifar10', 'svhn', 'fmnist'}:
        n_classes = 10
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                # Compute loss
                loss = 0.5 * criterion(out, combined_target)
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                if mask_clean.sum() > 0:
                    align_loss_clean = 1 * criterion_bce(sim_global[mask_clean] / temperature, torch.ones(mask_clean.sum()).cuda())
                else:
                    align_loss_clean = 0.0

                # 对齐损失（后门样本对齐 backdoor_net）
                if mask_backdoor.sum() > 0:
                    align_loss_backdoor = 10 * criterion_bce(sim_backdoor[mask_backdoor] / temperature, torch.ones(mask_backdoor.sum()).cuda())
                else:
                    align_loss_backdoor = 0.0
                
                # 对抗性损失（良性样本远离 backdoor_net）
                if mask_clean.sum() > 0:
                    adv_loss = 10 * criterion_bce(sim_backdoor[mask_clean] / temperature, torch.zeros(mask_clean.sum()).cuda())
                else:
                    adv_loss = 0.0

                # 分类损失
                loss_cls = 0.5 * criterion_cls(out, combined_target)
                ...
                # 总损失
                loss = loss_cls + fedprox_reg + align_loss_clean + align_loss_backdoor + adv_loss
                #loss = align_loss_clean + loss_cls + fedprox_reg
                #loss = loss_cls
...
def train_net_adv(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                # 总损失
                loss = loss_cls + adv_loss #+ fedprox_reg + align_loss_clean + align_loss_backdoor
                #loss = align_loss_clean + loss_cls + fedprox_reg
                #loss = loss_cls
        ...
        if net_id == 0:
            ...
            epoch_loss_cls_collector = sum(epoch_loss_cls_collector) / len(epoch_loss_cls_collector)
            epoch_adv_loss_collector = sum(epoch_adv_loss_collector) / len(epoch_adv_loss_collector)
            logger.info('Round: %d Client: %d Epoch: %d Loss: %f loss_cls: %f adv_loss: %f' % (round, net_id, epoch, epoch_loss, epoch_loss_cls_collector, epoch_adv_loss_collector))
...
def train_net_align(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                # 总损失
                loss = loss_cls + align_loss_backdoor #+ align_loss_clean + adv_loss + fedprox_reg 
                #loss = align_loss_clean + loss_cls + fedprox_reg
                #loss = loss_cls
        ...
        if net_id == 0:
            ...
            epoch_align_loss_clean_collector = sum(epoch_align_loss_clean_collector) / len(epoch_align_loss_clean_collector)
            epoch_align_loss_backdoor_collector = sum(epoch_align_loss_backdoor_collector) / len(epoch_align_loss_backdoor_collector)
            logger.info('Round: %d Client: %d Epoch: %d Loss: %f loss_cls: %f align_loss_clean: %f align_loss_backdoor: %f ' % (round, net_id, epoch, epoch_loss, epoch_loss_cls_collector, epoch_align_loss_clean_collector, epoch_align_loss_backdoor_collector))
...
def local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model=None, prev_model_pool=None, server_c = None, clients_c = None, round=None, device="cuda:0", backdoor_model=None):
    ...
    for net_id, net in nets.items():
        ...
        if args.backdoor == 'backdoor_MCFL':#and net_id == 0:
            '''
            prev_models=[]
...
def backdoor_MCFL(args):
    ...
    if args.backdoor == 'backdoor_MCFL':
        ...
        for round in range(n_comm_rounds):
            ...
            if args.fedavg_method == 'weight_fedavg':
                ...
                for net_id, net in enumerate(nets_this_round.values()):
                    ...
                    if net_id == 0:
                        ...
                        for key in net_para:
                            # 确保 net_para[key] 在与 global_w[key] 相同的设备上
                            net_para[key] = net_para[key].to(global_w[key].device)
                            global_w[key] += net_para[key] / num_clients
            ...
            elif args.defence == 'lockdown':
                lockdown_aggregate(global_model, nets_this_round, net_dataidx_map, k=10, device='cuda:0')
            ...
            # 假设 test_dl_global 可以作为验证集 (或者您可以单独准备一份干净/验证集)
            all_results, suspicious_label = neural_cleanse_br5(
                model=global_model,
                dataloader=test_dl_global,
                device=device,
                num_classes=10,
                square_size=5,
                max_steps=200,
