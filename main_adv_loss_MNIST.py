...
def get_args():
    ...
    parser.add_argument('--log_file_name', type=str, default='MNIST_resnet50_MCFL_BadNets_fedavg', help='The log file name')
...
def train_net_fedcon_backdoor_bad(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                # 归一化特征表示
                b_pro1 = torch.nn.functional.normalize(b_pro1, dim=-1)
                b_pro_backdoor = torch.nn.functional.normalize(b_pro_backdoor, dim=-1)
                ...
                for previous_net in previous_nets:
                    ...
                    b_pro3 = torch.nn.functional.normalize(b_pro3, dim=-1)
                    b_nega = cos(b_pro1, b_pro3)
                    b_logits = torch.cat((b_logits, b_nega.reshape(-1,1)), dim=1)

                # 创建标签
                num_negatives = b_logits.size(1) - 1
                b_labels = torch.cat([
                    torch.ones(backdoor_x.size(0), 1),
                    torch.zeros(backdoor_x.size(0), num_negatives)
                ], dim=1).cuda()

                # 计算对齐损失
                b_logits /= temperature
                loss4 = mu * criterion_bce(b_logits, b_labels)

                # 计算分类损失
                loss3 = criterion_cls(b_out, backdoor_target)

                # 计算后门样本的准确率
                _, b_predicted = torch.max(b_out, 1)
                correct_backdoor += (b_predicted == backdoor_target).sum().item()
                total_backdoor += backdoor_target.size(0)
                ...
                # 计算 net、global_net 和 backdoor_net 的输出
                _, pro1, out = net(clean_x)
                _, pro2, _ = global_net(clean_x)
                _, pro_backdoor_clean, _ = backdoor_net(clean_x)

                # 归一化特征表示
                pro1 = torch.nn.functional.normalize(pro1, dim=-1)
                pro2 = torch.nn.functional.normalize(pro2, dim=-1)
                pro_backdoor_clean = torch.nn.functional.normalize(pro_backdoor_clean, dim=-1)
                ...
                # 创建标签
                num_negatives = logits.size(1) - 1
                labels = torch.cat([
                    torch.ones(clean_x.size(0), 1),
                    torch.zeros(clean_x.size(0), num_negatives)
                ], dim=1).cuda()

                # 计算对齐损失
                logits /= temperature
                loss2 = mu * criterion_bce(logits, labels)

                # 计算对抗性损失（良性样本远离 backdoor_net）
                sim_backdoor_clean = cos(pro1, pro_backdoor_clean)
                adv_loss = mu * criterion_bce(sim_backdoor_clean / temperature, torch.zeros_like(sim_backdoor_clean))

                # 计算分类损失
                loss1 = criterion_cls(out, clean_target)

                # 计算良性样本的准确率
                _, predicted = torch.max(out, 1)
                correct_clean += (predicted == clean_target).sum().item()
                total_clean += clean_target.size(0)

                '''
                总损失
                '''
                loss = loss1 + loss2 + loss3 + loss4 + adv_loss
            ...
            for batch_idx, (x, target) in enumerate(train_dataloader):
                ...
                labels = torch.cat([
                    ...
                    torch.zeros(x.size(0), num_negatives)
                ], dim=1).cuda()

                logits /= temperature
                loss2 = mu * criterion_bce(logits, labels)
                loss1 = criterion_cls(out, target)
                loss = loss1 + loss2

                # 计算良性样本的准确率
                _, predicted = torch.max(out, 1)
                correct_clean += (predicted == target).sum().item()
                ...
                wandb.log({
                    'Round': round,
                    'benign_loss_total': loss.item(),
                    'benign_loss_1': loss1.item(),
                    'benign_loss_2': loss2.item(),
                })
        ...
        if net_id == 0:
            ...
            logger.info('Round: %d Client: %d Epoch: %d Loss: %f Loss1: %f Loss2: %f Loss3: %f Loss4: %f Adv_Loss: %f' % (round, net_id, epoch, epoch_loss, epoch_loss1, epoch_loss2, epoch_loss3, epoch_loss4, epoch_adv_loss))
            ...
            wandb.log({
                ...
                'attack_adv_loss': epoch_adv_loss,
                'train_clean_acc': train_clean_acc,
                'train_backdoor_acc': train_backdoor_acc,
            })
        else:
            ...
            wandb.log({
                'Round': round,
                'benign_loss_total': epoch_loss,
                'benign_loss_1': epoch_loss1,
                'benign_loss_2': epoch_loss2,
                'train_clean_acc': train_clean_acc,
            })
