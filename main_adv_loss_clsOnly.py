...
def get_args():
    ...
    parser.add_argument('--optimizer', type=str, default='amsgrad', help='the optimizer')
    ...
    parser.add_argument('--weight_align_lambda', type=float, default=0.1,
                    help='Weight alignment regularization strength')
    ...
    if args.wandb:
        wandb.init(
            ...
            'weight_align_lambda': args.weight_align_lambda
            }
        )
...
def train_net_nocls(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                ...
                if mask_backdoor.sum() > 0:
                    align_loss_backdoor = 1 * criterion_bce(sim_backdoor[mask_backdoor] / temperature, torch.ones(mask_backdoor.sum()).cuda())
                else:
                    align_loss_backdoor = 0.0
                
                # 对抗性损失（良性样本远离 backdoor_net）
                if mask_clean.sum() > 0:
                    adv_loss = 1 * criterion_bce(sim_backdoor[mask_clean] / temperature, torch.zeros(mask_clean.sum()).cuda())
                else:
                    adv_loss = 0.0
                ...
                # 总损失
                loss = fedprox_reg + align_loss_clean + align_loss_backdoor + adv_loss
        ...
        if net_id == 0:
            ...
            logger.info('Round: %d Client: %d Epoch: %d Loss: %f align_loss_clean: %f align_loss_backdoor: %f adv_loss: %f' % (round, net_id, epoch, epoch_loss, epoch_align_loss_clean_collector, epoch_align_loss_backdoor_collector, epoch_adv_loss_collector))
...
def train_net_fedcon_backdoor(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        if net_id == 0:
            ...
            for i in range(1): # 对齐强度
                train_dataloader = tqdm(train_dataloader)
                train_dataloader.set_description(f"Training traindata cleandata | round:{round} client:{net_id} re-train:{i}")
...
# 添加了直接使用net和backdoor model进行对齐
def train_net_fedcon_weight_align_backdoor(net_id, net, global_net, previous_nets, backdoor_net, train_dataloader, test_dataloader, backdoor_train_dl, backdoor_test_dl, epochs, lr, args_optimizer, mu, temperature, args, round, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        epoch_align_loss_backdoor_collector = []
        epoch_adv_loss_collector = []
        epoch_weight_align_loss_collector = []
        ...
        if net_id == 0:
            ...
            for i in range(1): # 对齐强度
                ...
                for batch_idx, ((clean_x, clean_target), (backdoor_x, backdoor_target)) in enumerate(zip(train_dataloader, backdoor_train_dl)):
                    ...
                    # +++ 新增参数对齐损失 +++
                    if net_id == 0 and args.weight_align_lambda > 0:
                        weight_align_loss = 0.0
                        # 注意：使用module获取原始模型参数（当使用DataParallel时）
                        for param_current, param_backdoor in zip(net.module.parameters(), backdoor_net.parameters()):
                            weight_align_loss += torch.norm(param_current - param_backdoor, p=2) ** 2
                        
                        weight_align_loss = args.weight_align_lambda * weight_align_loss
                        loss += weight_align_loss
                        epoch_weight_align_loss_collector.append(weight_align_loss.item())
        ...
        if net_id == 0:
            ...
            epoch_align_loss_backdoor_collector = sum(epoch_align_loss_backdoor_collector) / len(epoch_align_loss_backdoor_collector)
            epoch_adv_loss_collector = sum(epoch_adv_loss_collector) / len(epoch_adv_loss_collector)
            epoch_weight_align_loss = sum(epoch_weight_align_loss_collector)/len(epoch_weight_align_loss_collector) if epoch_weight_align_loss_collector else 0.0
            
            logger.info('Round: %d Client: %d Epoch: %d Loss: %f loss_cls: %f align_loss_clean: %f align_loss_backdoor: %f adv_loss: %f' % (round, net_id, epoch, epoch_loss, epoch_loss_cls_collector, epoch_align_loss_clean_collector, epoch_align_loss_backdoor_collector, epoch_adv_loss_collector))
            logger.info('Weight Align Loss: %f' % epoch_weight_align_loss)
            ...
            if args.wandb:
                wandb.log({
                    ...
                    'train_clean_acc': train_clean_acc,
                    'train_backdoor_acc': train_backdoor_acc,
                    'weight_align_loss': epoch_weight_align_loss,
                })
